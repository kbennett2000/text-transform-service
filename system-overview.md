...
5. **GPU phase exclusivity:** only one workload uses the 5070 at a time. As of T22 (text-transform-service ADR-0009) this is enforced *server-side* by the two on-box GPU tenants (text-transform-service and imagegen-service), which coordinate through one shared advisory `flock` (`/run/gpu-tenant.lock`): whoever holds it owns the GPU, drains its queued work under one lease, frees its own VRAM, then releases. The lock is fail-open (a throughput optimization, not a safety gate). The i5/Scriptorium orchestrator still sequences its own render/LLM phases, but exclusivity no longer depends on the caller getting that sequencing right.
